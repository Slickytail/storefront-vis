  ...
  dataTable <- reactive({
    ...
    if (input$variable2 != "NONE" && input$variable2 != input$variable) {
      ...
      sc <- s %>%
        group_by(v, v2) %>%
        summarise(n = sum(w)) %>%
        mutate(n = n/map_dbl(factor(v2), function(c, k) {k[[c]]}, v2D)) %>%
  ...
  barPlot <- reactive({
    if (input$variable2 != "NONE" && input$variable2 != input$variable) {
      ggplot(data = barPlotData())  +
        geom_bar(aes(x=v, y=co, fill=v2), stat="identity", position = "fill") +
  ...
  barPlotData <- reactive({
    ...
    if (input$variable2 != "NONE" && input$variable2 != input$variable) {
      ...
      return (s %>%
        ...
        group_by(v, v2) %>%
        summarise(co = sum(w)) %>%
        mutate(co = switch(input$distrib.mode, "real" = co, "sample"=, "set"=co/map_dbl(factor(v2), function(c, k) {k[[c]]}, r))))
    } 
    else {
